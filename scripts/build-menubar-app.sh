...
VERSION="${VERSION#v}"
BUNDLE_ID="${BUNDLE_ID:-io.github.antigravity-proxy.launcher.menubar}"
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/build-menu}"
APP_PATH="$OUTPUT_DIR/Antigravity Proxy Menu.app"
...
/usr/bin/plutil -lint "$ROOT_DIR/packaging/MenuBar-Info.plist" >/dev/null
...
/usr/bin/install -m 644 "$ROOT_DIR/packaging/MenuBar-Info.plist" "$CONTENTS/Info.plist"
...
printf 'Version: %s\n' "$VERSION"
printf 'Bundle ID: %s\n' "$BUNDLE_ID"
printf 'This is a preview build and does not replace the stable launcher.\n'
